...
- **Un défaut latent trouvé par ces contrôles** : deux `getCmd()` de l'ajax ne nommaient pas le type de commande recherché. C'est la cause du bug « Une commande portant ce nom (Reboot) existe déjà », corrigé partout ailleurs mais oublié à cet endroit. Aucun des deux identifiants concernés n'a aujourd'hui de jumeau d'un autre type : le défaut ne se serait réveillé qu'à la première commande action nommée comme l'une d'elles. L'équipement et les commandes sont vérifiés au passage, là où un `false` aurait produit une erreur fatale.

### 08/09/2026 (correctifs issus de l'audit du code)
- **Le widget SIP d'un compte non administrateur restait muet** avec la configuration par défaut : le garde de permission de la suppression des captures s'appliquait par erreur aux cinq actions ajax du plugin, dont celles que le widget appelle pour obtenir sa configuration et signaler un décrochage. Il ne s'applique plus qu'à la suppression.
- La page d'équipement affichait **deux onglets sécurité** : les types 1002 et 1110, ajoutés le 06/09, déclaraient leur section sans accent (« Securite »). Les alarmes porte/serrure et accès hors planning rejoignent l'onglet unique. Les actions déjà configurées sur ces types sont conservées — elles sont rangées par type, pas par onglet.
- En authentification MJPEG « Basic », un échec de connexion au portier écrivait **le mot de passe administrateur dans le log du serveur web** : l'URL du flux contenait les identifiants et partait telle quelle dans un warning PHP, hors de portée du masquage du log du plugin. Les identifiants passent désormais par un en-tête, l'URL ne porte plus aucun secret.
- **Une capture ratée n'est plus publiée comme une image.** Quand l'authentification échoue, le portier répond 200 avec son XML d'erreur : il était enregistré tel quel en `.jpg`, affiché comme « dernier snapshot » et transmis par « Envoyer un snapshot ». Le contenu est désormais vérifié (une image JPEG commence par `FF D8`) et le fichier fautif supprimé.
- Un équipement dont le nom contient « log », « backup » ou « scenario » — « Portier Logement », par exemple — **privait un simple utilisateur du téléchargement de toutes ses captures** : le filtre des fichiers réservés aux administrateurs cherchait ses motifs comme sous-chaînes du chemin complet, or le nom de l'équipement figure dans celui de chaque capture. Il compare désormais les segments du chemin et l'extension du fichier.
- Page de configuration : la coquille « Idenfiant » est corrigée, l'avertissement sur la protection par mot de passe devient traduisible (seule chaîne de la page à rester en français), et trois blocs ne partagent plus le même identifiant HTML — au passage, le `placeholder` vide du checkbox faisait dérailler l'extracteur de traductions, qui avalait la chaîne « Identifiant : ».
